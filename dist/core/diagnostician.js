 ...
 */
export function classifyLayer(url, prodHost) {
    if (!url)
        return "unknown";
    let parsed;
    try {
    ...
    try {
        ...
        if (parsed.hostname === prod.hostname)
            return "backend";
    }
    catch {
...
export function classifyAll(rawErrors, prodHost) {
...
export function runHeuristics(input) {
    ...
    if (supa406 && backend5xx) {
        return {
            diagnosis: `La capa Supabase devolvió 406 en ${shortPath(supa406.endpoint)} ` +
            ...
            recommendation: `Cambiar la query a .maybeSingle() y manejar el null con un fallback explícito. ` +
    ...
    if (supa406 && !has5xxAny) {
        return {
            diagnosis: `Supabase devolvió 406 en ${shortPath(supa406.endpoint)}. La query espera exactamente ` +
                `1 fila (.single() o Accept pgrst.object+json) y obtuvo 0. El frontend lo trata como error.`,
            recommendation: "Cambiar a .maybeSingle() y manejar el caso null en el componente que lee este recurso.",
            risks: [
    ...
    if (backend5xx) {
        return {
            diagnosis: `El backend devolvió ${backend5xx.status} en ${shortPath(backend5xx.endpoint)}. ` +
            ...
            recommendation: "Revisar logs del endpoint en Vercel/Cloudflare. Buscar try/catch que falte y devolver 4xx con mensaje en vez de 5xx silencioso.",
            risks: [
    ...
    if (backend404 && !backend5xx) {
        return {
            diagnosis: `El backend devolvió 404 en ${shortPath(backend404.endpoint)}. La ruta no existe en la app, ` +
                `o el id de la URL llega como null/undefined al montar el componente.`,
            recommendation: "Verificar que la ruta esté declarada (revisar app router) y que el id se cargue antes del fetch (skip el fetch si id es nullish).",
            risks: [
    ...
    if (supa401 || backend401) {
        ...
        return {
            diagnosis: `Recibimos 401 en ${shortPath(e.endpoint)} (${e.layer}). La sesión del usuario QA se perdió ` +
                `o el token expiró durante la prueba.`,
            recommendation: "Si pasa solo en QA: revisar duración del token de QA. Si pasa también en prod: la app debe refrescar el token automáticamente o redirigir a login con mensaje claro.",
            risks: [
    ...
    if (supa403) {
        return {
            diagnosis: `Supabase devolvió 403 en ${shortPath(supa403.endpoint)}. La RLS rechaza al rol del QA user ` +
                `(${input.featureName}).`,
            recommendation: "Revisar la policy RLS de la tabla. Decidir si el rol debe poder leer/escribir y ajustar la policy o el rol.",
            risks: [
 ...
 */
export async function diagnose(input, anthropic) {
    ...
    if (!anthropic) {
        return {
            diagnosis: `Se detectaron errores durante la prueba de ${input.featureName} pero no encajan en ningún patrón conocido. ` +
    ...
    try {
        const r = await anthropic.invoke({
            ...
            staticPrefix: `Eres un ingeniero senior diagnosticando bugs de QA. Recibes errores de red y consola ` +
            ...
            dynamicSuffix: `Errores observados:\n${dynamicSuffix}\n\nResponde SOLO con el JSON, sin texto adicional.`,
            maxTokens: 800,
            includeDefenseBlocks: true,
        });
        const out = anthropic.parseJson(r.text);
        return {
        ...
        return {
            diagnosis: `Errores detectados en ${input.featureName} pero el diagnóstico automático falló. ` +

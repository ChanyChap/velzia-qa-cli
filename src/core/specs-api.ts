...
import type { Spec, SpecState } from "../types/spec.js";
...
export class SpecsApi {
  ...
  async list(filter: { project?: string; state?: SpecState }): Promise<Spec[]> {
    ...
    if (!r.ok) throw new Error(`specs list ${r.status}: ${await r.text()}`);
    return r.json() as Promise<Spec[]>;
  }

  async upsert(spec: Partial<Spec>): Promise<{ ok: boolean; id: string }> {
    const r = await fetch(`${this.env.QA_VELZIA_API_URL}/api/specs`, {
      method: "POST",
      headers: this.headers(),
      body: JSON.stringify(spec),
    });
    if (!r.ok) throw new Error(`specs upsert ${r.status}: ${await r.text()}`);
    return r.json() as Promise<{ ok: boolean; id: string }>;
  }

  async patch(id: string, patch: Partial<Spec>): Promise<void> {
    const r = await fetch(`${this.env.QA_VELZIA_API_URL}/api/specs/${encodeURIComponent(id)}`, {
      method: "PATCH",
      headers: this.headers(),
      body: JSON.stringify(patch),
    });
    if (!r.ok) throw new Error(`specs patch ${r.status}: ${await r.text()}`);
  }
  ...
  async reportVerdict(
    ...
    if (newStreak >= 3) (patch as any).priority = 50;

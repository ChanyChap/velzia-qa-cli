...
function parseMatrix(slug: string, content: string): ParsedEntry[] {
  ...
  for (const line of lines) {
    ...
    if (trimmed.includes("---")) continue; // separador
...
function parseRegistry(slug: string, content: string): ParsedEntry[] {
  const out: ParsedEntry[] = [];
  ...
  for (const sec of sections) {
    ...
    let petition = "";
    let urlAffected: string | null = null;
    ...
    const urlMatch = sec.match(/\*\*URLs?\/vistas? afectadas?:\*\*[\s\S]*?-\s+`?([^\n`]+)/i)
      || sec.match(/`(\/[^`]+)`/);
    if (urlMatch) urlAffected = urlMatch[1].trim().split(" ")[0];
    ...
    out.push({
      ...
      description: petition || "(sin descripción)",
      url: urlAffected,
      qaFlow,
      priority,
    });
...
export async function runSyncSpecs(args: SyncArgs): Promise<void> {
  const env = loadEnv();
  ...
  if (args.dryRun) {
    for (const e of entries.slice(0, 5)) console.log(`  ${e.id} :: ${e.name}`);
    return;
  }
  ...
  for (const e of entries) {
    ...
    inserted++;
    if (inserted % 10 === 0) console.log(`  ${inserted}/${entries.length}...`);
  }

...
import type { Finding, Fix } from "../types/finding.js";
...
function applyFix(cwd: string, fix: Fix): { ok: boolean; reason?: string } {
  const filePath = path.isAbsolute(fix.path) ? fix.path : path.join(cwd, fix.path);
  if (!fs.existsSync(filePath)) return { ok: false, reason: `archivo no existe: ${fix.path}` };
  const content = fs.readFileSync(filePath, "utf8");
  const occ = content.split(fix.old).length - 1;
  const newOcc = fix.new ? content.split(fix.new).length - 1 : 0;
  if (occ === 0 && newOcc > 0) return { ok: true, reason: "idempotent: ya aplicado" };
  if (occ === 0) return { ok: false, reason: `'old' no encontrado en ${fix.path}` };
  if (occ > 1 && !fix.replaceAll) return { ok: false, reason: `'old' aparece ${occ} veces, marca replaceAll` };
  const replaced = fix.replaceAll ? content.split(fix.old).join(fix.new) : content.replace(fix.old, fix.new);
  fs.writeFileSync(filePath, replaced, "utf8");
  return { ok: true };
}

interface SastreArgs {
  cwd: string;
  dryRun?: boolean;
}

export async function runSastre(args: SastreArgs): Promise<void> {
  const env = loadEnv();
  ...
  if (!fs.existsSync(cfgPath)) throw new Error(`Falta ${cfgPath}`);
  const cfg = JSON.parse(fs.readFileSync(cfgPath, "utf8"));
  const projectSlug: string = cfg.slug;
  ...
  candidates = candidates.filter((f) => {
    ...
    for (const p of paths) {
      if (seenFiles.has(p)) return false;
    }
  ...
  for (const f of candidates) {
    let success = false;
    const fixSets: Fix[][] = [f.fixes, ...(f.fixesAlternatives || [])];
    for (let attempt = 0; attempt < Math.min(5, fixSets.length); attempt++) {
      ...
      for (const fx of fixes) {
        ...
        if (!r.ok) { console.log(`  fail: ${r.reason}`); allOk = false; break; }
  ...
  if (!prResult.ok) console.log(`[sastre] gh pr create failed: ${prResult.err}`);
